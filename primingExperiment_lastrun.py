...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/jacquessinger/Documents/Academics/Research/Research Proposals/Priming and Interoception/SnakeExperiment/primingExperiment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    image = visual.ImageStim(
        win=win,
        name='image', 
        image='SnakePhotos/grayscale-white-noise.png', mask=None, anchor='center',
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('imagePaths.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine Backward_Mask
        Backward_Mask = data.Routine(
            name='Backward_Mask',
            components=[image],
        )
        Backward_Mask.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # store start times for Backward_Mask
        Backward_Mask.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Backward_Mask.tStart = globalClock.getTime(format='float')
        Backward_Mask.status = STARTED
        thisExp.addData('Backward_Mask.started', Backward_Mask.tStart)
        Backward_Mask.maxDuration = None
        # keep track of which components have finished
        Backward_MaskComponents = Backward_Mask.components
        for thisComponent in Backward_Mask.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Backward_Mask" ---
        # if trial has changed, end Routine now
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        Backward_Mask.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                Backward_Mask.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in Backward_Mask.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in Backward_Mask.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Backward_Mask
        Backward_Mask.tStop = globalClock.getTime(format='float')
        Backward_Mask.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Backward_Mask.stopped', Backward_Mask.tStop)
        ...
        while continueRoutine:
            ...
            if prime.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if prime.status == STARTED:
                if frameN >= (prime.frameNStart + 1.5):
                    # keep track of stop time/frame for later
        ...
        # create an object to store info about Routine Forward_Mask
        Forward_Mask = data.Routine(
            name='Forward_Mask',
            components=[image_2],
        )
        Forward_Mask.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # store start times for Forward_Mask
        Forward_Mask.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Forward_Mask.tStart = globalClock.getTime(format='float')
        Forward_Mask.status = STARTED
        thisExp.addData('Forward_Mask.started', Forward_Mask.tStart)
        Forward_Mask.maxDuration = None
        # keep track of which components have finished
        Forward_MaskComponents = Forward_Mask.components
        for thisComponent in Forward_Mask.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Forward_Mask" ---
        # if trial has changed, end Routine now
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        Forward_Mask.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if image_2.status == STARTED:
                if frameN >= (image_2.frameNStart + 30):
                    # keep track of stop time/frame for later
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                Forward_Mask.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in Forward_Mask.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in Forward_Mask.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Forward_Mask
        Forward_Mask.tStop = globalClock.getTime(format='float')
        Forward_Mask.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Forward_Mask.stopped', Forward_Mask.tStop)
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED:
                ...
                if len(_key_resp_allKeys):
                    ...
                    # was this correct?
                    if (key_resp.keys == str(snake)) or (key_resp.keys == snake):
                        key_resp.corr = 1
                    else:
                        key_resp.corr = 0
